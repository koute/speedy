...
#[bench]
fn deserialization_speedy_megabyte_buffer_cow_non_primitive( b: &mut Bencher ) {
    ...
    buffer = black_box( buffer );
    b.iter( || {
        let deserialized: Cow< [Byte] > = Readable::read_from_buffer_owned( Endianness::NATIVE, &buffer ).unwrap();
        deserialized
    })
}
